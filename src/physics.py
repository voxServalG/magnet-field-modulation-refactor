...
def calculate_field_from_coils(coils_3d: list[tuple[np.ndarray, np.ndarray, float, float]], 
    ...
    Args:
        ...
        show_progress: If True, prints progress every 10% of loops.

    Returns:
        np.ndarray: Total magnetic field B. Shape (M, 3).
    '''
    total_B = np.zeros_like(target_points)
    num_coils = len(coils_3d)
    
    for idx, (top_loop, bottom_loop, i_top_raw, i_bot_raw) in enumerate(coils_3d):
        if show_progress and num_coils > 10 and (idx + 1) % (num_coils // 10) == 0:
            print(f"    -> Field Calculation Progress: {100*(idx+1)/num_coils:.0f}%")
