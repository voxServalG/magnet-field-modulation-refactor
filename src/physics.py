...
def calculate_field_from_coils(coils_3d: list[tuple[np.ndarray, np.ndarray, int]], 
                               target_points: np.ndarray,
                               current_I: float = 1.0,
    ...
    Iterates over all coil loops and their segments, summing the magnetic field contributions.

    Args:
        coils_3d (list): The list of 3D coils. Each item is (top_coil_coords, bottom_coil_coords, direction_sign).
        target_points (np.ndarray): (M, 3) coordinates of points where B-field is evaluated.
        current_I (float, optional): The magnitude of current in the coils (Amperes). Defaults to 1.0.
        use_shielding (bool, optional): If True, includes mirror currents. Defaults to True.

    Returns:
        np.ndarray: Total magnetic field B at each target point. Shape (M, 3).
    '''
    total_B = np.zeros_like(target_points)
    
    # Iterate over each coil assembly
    for top_loop, bottom_loop, direction_sign in coils_3d:
        
        # Determine loop currents
        # We rely on the path orientation (from contour) to define the current direction.
        # Bottom loop current is reversed relative to top loop for Bx gradient configuration
        I_top = current_I
        I_bottom = -I_top
        
        loops_to_process = [
            (top_loop, I_top),
            (bottom_loop, I_bottom) 
        ]
        ...
        for loop_coords, loop_current in loops_to_process:
            ...
            # Iterate over segments in the loop
            for k in range(num_points - 1):
                p_start = loop_coords[k]
                p_end = loop_coords[k+1]
                
                # Get segments (Original + Mirrors if enabled)
                if use_shielding:
